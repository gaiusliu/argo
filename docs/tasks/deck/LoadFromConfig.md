...
解析 `~/.argo/tools.json` 配置文件。CLI 列表：构造 cliTool 后 Register。MCP 列表：构造 MCPManager → Start → ListTools 获取清单 → 构造 mcpTool 后 Register。处理文件不存在 / JSON 格式错误等异常。
